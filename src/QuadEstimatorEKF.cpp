...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // q_t is the state quaternion from roll/pitch/yaw
  Quaternion<float> q_t = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  // dq to be the quaternion that consists of the measurement
  //   of the angular rates from the IMU in the body frame,
  // q_t_bar is the predicted quaternion
  // q_t_bar = d_q * q_t;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F accel_in_global_frame = attitude.Rotate_BtoI(accel);

  predictedState(3) = curState(3) + accel_in_global_frame.x * dt; // x_dot + (Rbg.x * x_dot_dot * dt)
  predictedState(4) = curState(4) + accel_in_global_frame.y * dt; // y_dot + (Rbg.y * y_dot_dot * dt)
  predictedState(5) = curState(5) - CONST_GRAVITY * dt + accel_in_global_frame.z * dt;// z_dot - (g*dt) + (Rbg.z * z_dot_dot * dt)
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  // Identity matrix: Set diagonals to 1
  hPrime(0, 0) = 1;
  hPrime(1, 1) = 1;
  hPrime(2, 2) = 1;
  hPrime(3, 3) = 1;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  // yaw -> -2*pi <= yaw <= 2*pi
  if (diff >  F_PI)  zFromX(0) = zFromX(0) + 2.f * F_PI;
  if (diff < -F_PI)  zFromX(0) = zFromX(0) - 2.f * F_PI;
